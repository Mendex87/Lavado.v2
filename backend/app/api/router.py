...
from app.api.routes import admin, alarms, audit, auth, dashboard, events, health, measurements, plc, processes, settings, stock
...
api_router.include_router(events.router, dependencies=[Depends(get_current_user), Depends(require_roles('admin'))])
api_router.include_router(alarms.router, dependencies=[Depends(get_current_user), Depends(require_roles('admin'))])
