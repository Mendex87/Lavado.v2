...
from app.api.routes import admin, auth, dashboard, events, health, measurements, plc, processes, simulation, stock
...
api_router.include_router(plc.router, dependencies=[Depends(get_current_user)])
api_router.include_router(simulation.router, dependencies=[Depends(get_current_user)])
